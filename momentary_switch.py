...
class MomentarySwitch(SwitchEntity):
    ...
    def __init__(self, config):
        ...
        if self._mode == "old":
            self._toggle_for = config.get(CONF_ON_FOR)  # default = 1 sec
            self._cancellable = config.get(CONF_ALLOW_OFF)  # default = False
            self._toggled = "on"
        ...
        print("MomentarySwitch: {} created".format(self._name))
